...
elseif colRef == 4
    timeseriesosimtable = TimeSeriesTableQuaternion();
end
...
elseif colRef == 4
    % Get an OpenSim Row Vector
    row = RowVectorQuaternion(nfields);
    for iRow = 1 : nRows
        % Create and fill a row of data
        for iCol = 1 : nfields 
            % Get the row value from each field of the struct
            currQuat = org.opensim.modeling.Quaternion();
            for iQuat = 1:4
                currQuat.set(iQuat-1, s.(labels{iCol})(iRow,iQuat))
            end
            row.set(iCol-1, currQuat)
        end
        % Append the Quaternion to the opensim table
        timeseriesosimtable.appendRow(iRow-1, row);
    end
end
